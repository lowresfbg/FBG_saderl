...
def FeatureModel():
    ...
    x = tf.keras.layers.Conv1D(50, 3, activation='sigmoid')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
    x = tf.keras.layers.Conv1D(20, 5, activation='sigmoid')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
    x = tf.keras.layers.Conv1D(10, 10, activation='sigmoid')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
    x = tf.keras.layers.Conv1D(10, 5, activation='sigmoid')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
