...
def CompensateModel():
    ...
    # x += tf.exp(-(input_layer[:, :1]/0.2)**2*tf.math.log(2.0)*4)*input_layer[:, 1:2]
...
def GetModel():
    ...
    compensates = tf.reshape(compensates, full_shape)* I

    compensates = tf.reduce_sum(compensates, axis=1)*0.001
