...
def Encoder():
    ...
    x = tf.keras.layers.Conv1D(100, 5, activation='relu', padding='same')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
    x = tf.keras.layers.Conv1D(100, 5, activation='relu', padding='same')(x)
    x = tf.keras.layers.MaxPooling1D(2)(x)
    x = tf.keras.layers.Conv1D(50, 5, activation='relu', padding='same')(x)
...
def Decoder():
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(10,activation='relu'))(x)
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(50,activation='relu'))(x)
    x = tf.keras.layers.Permute((2,1))(x)
