...
def Encoder():
    ...
    x = tf.keras.layers.Conv1D(100, 10, activation='sigmoid', padding='same')(x)
...
def Decoder():
    ...
    x = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(250,use_bias=False,activation='elu'))(x)
    x = tf.keras.layers.Permute((2,1))(x)
