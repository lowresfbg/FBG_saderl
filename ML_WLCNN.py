...
def Resample(skip=1, target=1000):
    dataset = tf.constant(Dataset(), dtype=tf.dtypes.float32)[::4, :, ::skip]
    ...
    answer = np.array(Get3FBGAnswer()).T[::4]
    return dataset, answer

dataset, answer = Resample(5,10000)
...
dataset, answer = Resample(1,1000)
...
def plot_ml(batch, logs):
    if batch%2==0:
        ...
        for i in range(train_Y[0].shape[0]):
            plt.plot(np.array(train_Y)[:,i], "o", label="DE-FBG{}".format(i+1))
...
sample_weight = (sample_weight - sample_weight_max)/(sample_weight_min-sample_weight_max)
